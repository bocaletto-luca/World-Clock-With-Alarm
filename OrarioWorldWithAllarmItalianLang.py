        ...
        try:
            ...
            if difference.total_seconds() > 0:
                ...
                def play_alarm():
                    ...
                    messagebox.showinfo("Sveglia", f"Sveglia impostata per {selected_time} in {selection}")
                ...
                alarm_status_label.config(text=f'Sveglia impostata per {selected_time} in {selection}', fg="green")
            else:
                ...
                alarm_status_label.config(text=f'L\'orario selezionato è già trascorso.', fg="red")
        except pytz.exceptions.UnknownTimeZoneError:
            ...
            alarm_status_label.config(text=f'Fuso orario sconosciuto per {selection}', fg="red")
    else:
...
# Create and configure UI elements (Crea e configura gli elementi dell'interfaccia utente)
title_label = tk.Label(app, text='Orologio Mondiale con Sveglia', font=("Helvetica", 16))
...
show_selected_button = tk.Button(app, text='Mostra Ora', command=show_selected_time)
...
# Periodically update the displayed time (Aggiorna periodicamente l'orario visualizzato)
app.after(1000, show_selected_time)
